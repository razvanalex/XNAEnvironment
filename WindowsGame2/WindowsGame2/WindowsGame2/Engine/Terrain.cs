...
namespace WindowsGame2
{
    public class Terrain : IRenderable
    {
            ...
            GraphicsDevice GraphicsDevice, ContentManager Content)
        {
            ...
            effect = Content.Load<Effect>("shaders//Terrain");
        ...
        public void SetModelEffect(Effect effect, bool CopyEffect)
        {
            ...
            MeshTag tag = ((MeshTag)effect.Tag);
            if (tag.Texture != null)
            {
              toSet.Parameters["BasicTexture"].SetValue(tag.Texture);
              toSet.Parameters["TextureEnabled"].SetValue(true);
            }
            else toSet.Parameters["TextureEnabled"].SetValue(false);

            toSet.Parameters["DiffuseColor"].SetValue(tag.Color);
            toSet.Parameters["SpecularPower"].SetValue(tag.SpecularPower);
        ...
        {
            ((MeshTag)effect.Tag).CachedEffect = effect.Clone();
           // ((MeshTag)indexBuffer.Tag).CachedEffect = indexBuffer;
        }
        public void RestoreEffects()
        {
            effect = ((MeshTag)effect.Tag).CachedEffect;
        }
